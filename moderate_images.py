...
1.  **이미지 품질 및 심미성 (Quality & Aesthetics):**
    ...
    *   **불가:** 저화질, 흐릿함, 심한 노이즈, 과도한 왜곡/보정, 복잡하거나 산만한 배경, 어둡거나 조명이 부적절한 이미지. 가로세로 비율이 이상하게 왜곡된 이미지.

2.  **콘텐츠 적절성 (Content Appropriateness):**
    *   **가능:**
        *   휴대폰으로 얼굴을 가린 컨셉 또는 거울에 비친 컨셉
        *   식사중 촬영한 컨셉
        *   의류 착용 상황을 부각시키는 자연스러운 상황
    *   **불가:**
        *   혐오/차별/비방: 혐오감을 주거나 사회적 이슈/차별/비방 소지가 있는 콘텐츠.
        *   허위/과장: 실제 상품과 다르거나 기능을 과장하는 이미지 (특히 Before/After 비교 이미지).
        *   기타: 담배 노출 이미지, 과도한 분할컷/합성 이미지. 분할컷은 서로 다른 성격의 이미지가 화면상 분할된 프레임으로 표현된 것이다.
        *   유명 연예인의 사진을 사용하는 경우
3. 선정성/과장성 판단 기준
    가능:
